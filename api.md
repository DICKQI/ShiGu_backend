...
| `group_by`    | string | **分组显示**：按指定字段分组显示谷子列表。可选值：`ip`（IP作品）、`character`（角色）、`category`（品类）、`theme`（主题）。使用此参数时返回分组格式，不使用时返回普通列表格式 |
| `page`        | int    | 分页页码，从 1 开始，例如 `?page=1` 表示第一页（**注意**：使用 `group_by` 参数时不支持分页）                                               |
| `page_size`   | int    | 每页数量，默认 18 条，最大 100 条，例如 `?page_size=50`（**注意**：使用 `group_by` 参数时不支持分页）                                      |
...
> 示例 7：按 IP 作品分组显示谷子：
>
> `/api/goods/?group_by=ip`（按 IP 作品分组显示所有谷子）
>
> 示例 8：按角色分组显示星铁的谷子：
>
> `/api/goods/?ip=1&group_by=character`（筛选星铁的谷子，并按角色分组显示）
>
> 示例 9：按品类分组显示流萤的谷子：
>
> `/api/goods/?character=5&group_by=category`（筛选流萤的谷子，并按品类分组显示）
...
当使用 `group_by` 参数时，返回格式如下：

**按 IP 作品分组示例**：

```http
GET /api/goods/?group_by=ip
```
...
```http
GET /api/goods/?group_by=character
```
...
**注意事项**：
- 使用 `group_by` 参数时，不支持分页（`page` 和 `page_size` 参数将被忽略）。
...
- **URL**：`GET /api/goods/similar-random/`
- **说明**：
  - 返回按相似度智能分组的谷子列表，而非完全随机展示。
  - 通过多维度加权评分算法（IP、角色、品类、主题、价格、入手日期）将相似的谷子整理在一起。
  - 提供更好的浏览体验，帮助用户发现相关的谷子。
  - 响应格式与标准列表接口（4.1）完全相同，支持分页和所有过滤器。

#### 相似度算法说明

算法基于6个维度的加权评分（总分100分）：

| 维度 | 权重 | 说明 |
| ---- | ---- | ---- |
| IP匹配 | 30分 | 相同IP得30分，相同作品类型得10分 |
| 角色重叠 | 25分 | 共享角色数 / 总角色数 × 25 |
| 品类层级 | 20分 | 相同叶子品类20分，相同父级12分，相同祖父级6分 |
| 主题匹配 | 10分 | 相同主题得10分 |
| 价格区间 | 8分 | 价格差异≤10%得8分，≤25%得5分，≤50%得3分 |
| 入手日期 | 7分 | 同月得7分，3个月内得5分，6个月内得3分 |

**分组策略**：
1. 从不同IP中选择种子谷子（确保多样性）
2. 为每个种子找到相似度≥40分的谷子组成分组
3. 强制多样性规则：不允许连续3个以上来自同一IP
...
| `page_size`   | int    | 每页数量，默认 18 条，最大 100 条                                                             |

**相似度专用参数**：

| 参数名          | 类型   | 说明                                                                                          |
| --------------- | ------ | --------------------------------------------------------------------------------------------- |
| `seed_strategy` | string | 种子选择策略，默认 `diverse`：<br>- `diverse`：多样化选择（从不同IP中选择，默认）<br>- `popular`：从热门IP中选择<br>- `recent`：从最近添加的谷子中选择 |

#### 响应示例

响应格式与标准列表接口（4.1）完全相同：
...
1. **发现页/推荐页**：展示相关谷子，帮助用户发现收藏中的关联内容
2. **浏览模式**：提供比完全随机更有意义的浏览体验
3. **组合筛选**：结合过滤器使用，例如：
   - `/api/goods/similar-random/?ip=1&seed_strategy=popular` - 查看某个IP下按热度分组的谷子
   - `/api/goods/similar-random/?status=in_cabinet&seed_strategy=recent` - 查看在馆谷子按最近添加分组

#### 性能说明

- 首次请求会计算相似度排序（<100个谷子约100ms，100-500个约500ms）
- 计算结果会缓存5分钟，后续分页请求直接使用缓存（<50ms）
- 对于<18个谷子的情况，自动降级为简单随机排序

#### 与标准列表接口的区别

| 特性 | 标准列表（4.1） | 相似随机（4.6） |
| ---- | --------------- | --------------- |
| 排序方式 | 按 `order` 和 `created_at` 排序 | 按相似度智能分组 |
| 适用场景 | 精确查找、管理 | 发现、浏览 |
| 性能 | 极快（直接数据库排序） | 首次较慢（需计算），后续快（缓存） |
| 过滤器 | 完全支持 | 完全支持 |
| 分页 | 标准分页 | 标准分页（基于缓存排序） |

---

### 4.7 谷子统计图表数据（Dashboard）
